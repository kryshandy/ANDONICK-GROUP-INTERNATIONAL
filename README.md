...
- **Panneau « ANDONICK — Contenu du site »** :
  ...
  - **Sections libres** : 6 emplacements supplémentaires (3 de type « texte », 3 de type « bannière » avec image ou fond violet) — titre, texte, bouton, lien ; une ligne = un champ. Ils s'insèrent dans l'ordre des sections (Structure & Ordre) et disparaissent si vides
  - **Formulaires & listes — FR / EN** : les champs des formulaires « Demander un devis » et « Être rappelé » (1 ligne = `Libellé|type|obligatoire|source` — types : texte, téléphone, e-mail, zone de texte, liste ; sources : services ou créneaux), texte de la page 404 (titre + message + bouton), libellés « Article précédent / suivant », description de référencement (meta description, par langue)
  - **Images** : photo du hero, photos des sections, **12 places de galerie** (6 remplies ; place vide = photo non affichée), **image de partage (Open Graph)** pour les réseaux sociaux
